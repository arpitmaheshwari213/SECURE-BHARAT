...
if(isset($_SESSION['user_auth']) AND isset($_SESSION['time_left'])) {
    ...
    $sql = "SELECT question_id FROM user_quiz_response WHERE user_id = $user_id AND level = 4 AND user_response IS NULL ORDER BY question_id ASC";
        ...
        //echo "no of rows ".$n."  ".$number_of_question_level_2;
          if($n == 0){
           ...
           extract($row); 
            ...
            $all_question_ids_array = getAllRandomQuestionIdFromResponse($user_id,4,$cxn);
            //print_r($all_question_ids_array);

            if($_SESSION['current_question_count'] >=0 and $_SESSION['current_question_count'] < $number_of_question_level_4) {
...
<body style="color:#009688">
  ...
  <div class="col-sm-10 col-md-6">
      <p class="lead" style="margin-bottom:0px"><?php echo round(($current_question_number+1)/($number_of_question_level_4)*100)."%"; ?> Completed</p>
    <div class="progress">
        <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo round(($current_question_number+1)/($number_of_question_level_4)*100)."%"; ?>" aria-valuemin="0" aria-valuemax="<?php echo $number_of_question_level_4 ?>" style="width: <?php echo round(($current_question_number+1)/($number_of_question_level_4)*100)."%"; ?>;">
            <span class="sr-only"><?php echo round(($current_question_number+1)/($number_of_question_level_4)*100)."%"; ?> Completed</span>
      </div>
    </div>
